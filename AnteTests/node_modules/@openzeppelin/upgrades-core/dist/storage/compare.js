...
class StorageLayoutComparator {
    constructor(unsafeAllowCustomTypes = false) {
    ...
    layoutLevenshtein(original, updated, { allowAppend }) {
        const ops = levenshtein_1.levenshtein(original, updated, (a, b) => this.getFieldChange(a, b));
        if (allowAppend) {
    ...
    uncachedGetTypeChange(original, updated, { allowAppend }) {
        ...
        if (original.args === undefined || updated.args === undefined) {
            ...
            assert_1.assert(original.args === updated.args);
            return undefined;
        }
        switch (original.head) {
            ...
            case 't_struct': {
                ...
                assert_1.assert(layout_1.isStructMembers(originalMembers) && layout_1.isStructMembers(updatedMembers));
            ...
            case 't_enum': {
                ...
                assert_1.assert(layout_1.isEnumMembers(originalMembers) && layout_1.isEnumMembers(updatedMembers));
                if (enumSize(originalMembers.length) !== enumSize(updatedMembers.length)) {
                    return { kind: 'enum resize', original, updated };
                }
                else {
                    const ops = levenshtein_1.levenshtein(originalMembers, updatedMembers, (a, b) => a === b ? undefined : { kind: 'replace', original: a, updated: b }).filter(o => o.kind !== 'append');
                    if (ops.length > 0) {
            ...
            case 't_mapping': {
                ...
                assert_1.assert(is_value_type_1.isValueType(originalKey) && is_value_type_1.isValueType(updatedKey));
            ...
            case 't_array': {
                ...
                assert_1.assert(originalLength !== undefined && updatedLength !== undefined);
